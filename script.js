...
function setupIndependentScrolling() {
    ...
    // Variable to track if we're scrolling inside the menu
    let isScrollingMenu = false;
    
    // Track touch start inside menu
    flyoutMenu.addEventListener('touchstart', () => {
    ...
    // Prevent background scrolling without preventing menu scrolling
    document.addEventListener('touchmove', (e) => {
        // If menu is open and we're not scrolling inside the menu
        if (flyoutMenu.classList.contains('open') && !isScrollingMenu) {
            e.preventDefault();
        }
    }, { passive: false });
    
    // Handle edge cases when scrolling reaches the top or bottom of the menu
